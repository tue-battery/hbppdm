function [OCPval] = eco_night_attempt_maastricht(OCPval, w, mstr_data)
...
veh_par.Cd = 2.7338; %1.225*0.577*0.5*8.7975;% Total value for air resistance [-]
veh_par.Cr = 0.00627;                % vehicle friction drag [-]
veh_par.mu = 0.35;                  % friction coefficient 
veh_par.g = 9.81;                   % gravitational force [m/s^2]
veh_par.Fmax = 550000*x_scale;      % Newtons
%veh_par.vmax = 100/3.6;            % Newtons
veh_par.vmin = 15/3.6;               % Newtons

xinit = 0.5*veh_par.vmin.^2*veh_par.m*x_scale; % 1 MJ
...
if isempty(OCPval)
    ...
    %% State & input box - constraints
    vmax = (mstr_data.vmax(2:end)./3.6)';                  % N+1×1
    vmin = veh_par.vmin * ones(N,1);   % N+1×1
    
    % bounds on states per stap
    xub_i = x_scale * 0.5 * veh_par.m .* (vmax.^2);   % N×1
    xlb_i = x_scale * 0.5 * veh_par.m .* (vmin.^2);   % N×1
    
    % bounds on inputs per stap (3 inputs)
    Fmax = veh_par.Fmax;
    
    uubt_i = [ Fmax * ones(N,1),  1./vmin,  inf(N,1) ];           % N×3
    ulbt_i = [ -Fmax * ones(N,1), 1./vmax,  zeros(N,1) ];         % N×3
    
    % Combineer per stap: [state; inputs] -> (1+3)×N matrix, daarna flattenen naar vector
    WUB = [xub_i, uubt_i].';   % (1+3)×N
    WLB = [xlb_i, ulbt_i].';   % (1+3)×N
    
    wub = [WUB(:); xub_i(end)];  % append terminal state bound (hier gekozen als laatste stap)
    wlb = [WLB(:); xlb_i(end)];
    ...
    %% Height profile
    height = mstr_data.height(1:end);
    d_height = diff(height);
    alpha = min(0.15,max(-0.15,[atan(d_height/Ts)']));
